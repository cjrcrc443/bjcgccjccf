...
from AlinaXIQ.utils.filters_func import (
    GetFIlterMessage,
    SendFilterMessage,
    get_text_reason,
)
...
async def _filter(client, message):
    ...
    await message.reply(f"**چات زیادکرا بە ناوی ↤︎ `{filter_name}` ♥•**")
...
async def _filters(client, message):
    ...
    if len(FILTERS) == 0:
        await message.reply(f"**هیچ چاتێکی زیادکراو نییە لە {chat_title} ♥•**")
        return
...
async def stopall(client, message):
        ...
        [
            [
                InlineKeyboardButton(
                    text="سڕینەوەی هەموو چاتەکان", callback_data="custfilters_stopall"
                )
            ],
            [
                InlineKeyboardButton(
                    text="هەڵوەشانەوە", callback_data="custfilters_cancel"
                )
            ],
        ]
    )
    ...
    await message.reply(
        text=(
            f"**ئایا دڵنیای کە دەتەوێت هەموو چاتە زیادکراوەکان لە ئەم کردارەدا بوەستێنیت؟♥•**"
        ),
...
async def stopall_callback(client, callback_query: CallbackQuery):
    ...
    if query_data == "stopall":
        ...
        await callback_query.edit_message_text(
            text="**بە سەرکەوتوویی هەموو چاتەکان سڕدرانەوە♥️✅**"
        )

    elif query_data == "cancel":
        await callback_query.edit_message_text(
            text="**بە سەرکەوتوویی هەڵوەشێنرایەوە♥️✅**"
        )


@app.on_message(
    filters.command(["stopfilter", "stopchat", "rfilter", "rchat"]) & admin_filter
)
@user_admin
async def stop(client, message):
    ...
    await message.reply(
        f"**بە سەرکەوتوویی چاتە زیادکراوەکە سڕایەوە: `{filter_name}`♥•**"
    )
