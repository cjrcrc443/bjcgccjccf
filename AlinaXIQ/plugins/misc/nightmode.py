...
from AlinaXIQ.mongo.nightmodedb import (
    get_nightchats,
    nightdb,
    nightmode_off,
    nightmode_on,
...
async def _nightmode(_, message):
    return await message.reply_photo(
        ...
        caption="**✧¦کلیك بکە لە یەکێك لە دوگمەکان بۆ چالاککردن و ناچالاککردنی دۆخی شەو فەرمانەکە بۆ داخستن و کردنەوەی گرووپە بە خودکار🖤•**",
        ...
        if data == "add_night":
            if check_night:
                ...
                await query.message.edit_caption(
                    "**✧¦دۆخی شەو لەم گرووپە زیادکرا بۆ داتا بیسم، گرووپ کاتژمێر 𝟏𝟐ی شەو دادەخرێت وە کاتژمێر 𝟖ی بەیانی دەکرێتەوە**"
                )
        if data == "rm_night":
            if check_night:
                await nightmode_off(chat_id)
                await query.message.edit_caption(
                    "**✧¦زانیاری دۆخی شەو سڕدرایەوە لە داتا بەیسم**"
                )
            elif not check_night:
...
async def _nightmode(_, message):
    return await message.reply_photo(
        photo="https://telegra.ph//file/06649d4d0bbf4285238ee.jpg",
        caption="**✧¦کلیك بکە لە یەکێك لە دوگمەکان بۆ چالاککردن و ناچالاککردنی دۆخی شەو فەرمانەکە بۆ داخستن و کردنەوەی گرووپە بۆ خودکار🖤•**",
        reply_markup=buttons,
    )
