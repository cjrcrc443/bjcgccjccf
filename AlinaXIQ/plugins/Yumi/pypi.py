...
def pypi_info_command(client, message):
    try:
        ...
        if pypi_info:
            ...
            info_message = (
                f"𝗣𝗔𝗖𝗞𝗔𝗚𝗘 𝗡𝗔𝗠𝗘➪ {pypi_info['info']['name']}\n\n"
            ...
            client.send_message(
                message.chat.id, "Failed to fetch information from PyPI."
            )

    except IndexError:

        client.send_message(
            message.chat.id, "Please provide a package name after the /pypi command."
        )
