...
@app.on_message(command(["داخستنی وەڵامدانەوە"]) & filters.group)
async def iddlock(client: Client, message: Message):
    ...
    if get.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] and dev:
        ...
        return await message.reply_text(
            f"**بە سەرکەوتوویی فەرمانی وەڵامدانەوە داخرا\n\n لەلایەن {rotba} ←{message.from_user.mention}♥️❎•**"
        )
    else:
        return await message.reply_text(
            f"** {message.from_user.mention} تۆ ئەدمین نیت لێرە💔•**"
        )


@app.on_message(command(["کردنەوەی وەڵامدانەوە"]) & filters.group)
...
def call_random_member(client: Client, message: Message):
    ...
    members = [
        member for member in client.get_chat_members(chat_id) if not member.user.is_bot
    ]
    if chat_id in iddof:
        return
    random_member = random.choice(members)
    random_member_mention = (
        f"[{random_member.user.first_name}](tg://user?id={random_member.user.id})"
    )
