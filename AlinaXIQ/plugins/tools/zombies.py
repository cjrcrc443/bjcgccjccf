    ...
    try:
        ...
        if has_permissions:
            ...
            if bot.status == ChatMemberStatus.MEMBER:
                await message.reply(
                    "**➠ | پێویستە ڕۆڵم هەبێت بۆ سڕینەوەی هەموو ئەکاونتە سووتاوەکان⚡🖤•**"
                )
            else:
                if len(chatQueue) > 30:
                    await message.reply(
                        "**➠ | من دووبارە کاردەکەم گەورەترین ژمارەی گرووپ 30یە لەیەك کاتدا، تکایە دووبارەی بکەوە🖤•**"
                    )
                else:
                    if message.chat.id in chatQueue:
                        await message.reply(
                            "**➠ | پڕۆسەکە دووبارە کراوەتەوە لەم گرووپە، تکایە [ /stop ] بکە بۆ دەستپێکردنی دانەیەکی نوێ♥•**"
                        )
                    else:
                        ...
                        if lenDeletedList == 0:
                            await message.reply(
                                "**⟳ | هیچ ئەکاونتێکی سووتاو لەم گرووپە نییە🖤•**"
                            )
                            ...
                            temp = await app.send_message(
                                message.chat.id,
                                f"**🧭 | کۆی گشتی لە {lenDeletedList} ئەکاونتی سووتاو دۆزرایەوە\n🥀 | کاتی خەڵمێنراو: {processTime} چرکە لە ئێستا🖤•**",
                            )
                            if stopProcess:
                                stopProcess = False
                            ...
                            if k == lenDeletedList:
                                await message.reply(
                                    f"**✅ | بە سەرکەوتوویی سڕدرانەوە، هەموو ئەکاونتە سووتاوەکان لەم گرووپە🖤•**"
                                )
                                await temp.delete()
                            else:
                                await message.reply(
                                    f"**✅ | بە سەرکەوتوویی سڕدرایەوە {k} ئەکاونتی سووتاو لەم گرووپە🖤•**"
                                )
                                await temp.delete()
                            chatQueue.remove(message.chat.id)
        else:
            await message.reply(
                "**👮🏻 | ببوورە، تەنیا ئەدمینەکان دەتوانن ئەم فەرمانە بەکاربێنن🗿•**"
            )
