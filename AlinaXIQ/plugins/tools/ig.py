...
async def download_and_send_video(client, message):
    if len(message.command) < 2:
        await message.reply(
            "Please provide a valid video link. Usage: `/download <video_url>`"
        )
        return
...
async def download_video(video_url: str, chat_id: int, client, downloading_message):
    ...
    try:
        ...
        await client.send_message(
            chat_id, "Failed to download video. Please try again later."
        )
        print(f"Error: {e}")
    finally:
        # Delete the processing message
        await client.delete_messages(
            chat_id=chat_id, message_ids=[downloading_message.id]
        )
