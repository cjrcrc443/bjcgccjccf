...
from AlinaXIQ.utils import bot_sys_stats, bot_up_time
...
async def start_gp(client, message: Message, _):
    ...
    try:
        ...
        message = await message.reply_text(
            f"**🧑🏻‍💻┋ پشکنین بۆ [یاریدەدەرەکەم](tg://openmessage?user_id={userbot.id}) لە گرووپە یان نا •**"
        )
        is_userbot = await app.get_chat_member(message.chat.id, userbot.id)
        if is_userbot:
            await message.edit_text(
                f"**🧑🏻‍💻┋ [یاریدەدەرەکەم](tg://openmessage?user_id={userbot.id}) لە گرووپە ئێستا دەتوانی گەڕان بکەیت بۆ گۆرانی •**"
            )
    except Exception as e:
        # Userbot is not in the group, invite it
        try:
            await message.edit_text(
                f"**🧑🏻‍💻┋ [یاریدەدەرەکەم](tg://openmessage?user_id={userbot.id}) لە گرووپ نییە بانگهێشتی دەکەم ..**"
            )
            ...
            await message.edit_text(
                f"**🧑🏻‍💻┋ ناتوانم [یاریدەدەرەکەم](tg://openmessage?user_id={userbot.id}) بانگهێشت بکەم\nبمکە ئەدمین تاوەکو بتوانم زیادی بکەم •**"
            )
            ...
            if member.id == app.id:
                ...
                try:
                    ...
                    if message.chat.username:
                        ...
                        await message.reply_text(
                            f"**🧑🏻‍💻┋ [یاریدەدەرەکەم](tg://openmessage?user_id={userbot.id}) هاتە گرووپەوە بەهۆی یوزەری گرووپ •**"
                        )
                    else:
                        invitelink = await app.export_chat_invite_link(chid)
                        await asyncio.sleep(1)
                        messages = await message.reply_text(
                            f"**🧑🏻‍💻┋ [یاریدەدەرەکەم](tg://openmessage?user_id={userbot.id}) جۆین دەکات بە بەکارهێنانی لینك •**"
                        )
                        await userbot.join_chat(invitelink)
                        await messages.delete()
                        await message.reply_text(
                            f"**🧑🏻‍💻┋ [یاریدەدەرەکەم](tg://openmessage?user_id={userbot.id}) هاتە گرووپەوە بەهۆی لینکی گرووپ •**"
                        )
                except Exception as e:
                    await message.edit_text(
                        f"**🧑🏻‍💻┋ تکایە بمکە ئەدمین بۆ بانگهێشت کردنی [یاریدەدەرەکەم](tg://openmessage?user_id={userbot.id}) بۆ گرووپ •**"
                    )
