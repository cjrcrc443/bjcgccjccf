...
async def purge(app: app, msg: Message):
    ...
    if msg.chat.type != ChatType.SUPERGROUP:
        await msg.reply_text(
            text="**ناتوانم نامەکان پاکبکەمەوە لە گرووپێکی بنەڕەتیدا گرووپێکی گشتی دروست بکە🖤•**"
        )
        return
    ...
    if msg.reply_to_message:
        ...
        try:
            ...
            await msg.reply_text(
                text="**من ناتوانم هەموو نامەکان بسڕمەوە، لەوانەیە نامەکان کۆن بن یان لەوانەیە مافی یان ڕۆڵی سڕینەوەم نەبێ یان گرووپێکی گشتی نەبێ‌🖤•**"
            )
            return

        except RPCError as ef:
            await msg.reply_text(
                text=f"**هەندێك هەڵە هەیە, ڕیپۆرتی بکە بەبەکارهێنانی** `/bug`<b>هەڵە:</b> <code>{ef}</code>"
            )
